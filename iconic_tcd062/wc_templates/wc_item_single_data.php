...
<?php
	...
	elseif ( $metadata = usces_dlseller_validity( $post ) ) :
		$dlseller_table .= '<tr><th>' . __( 'dlValidity(days)', 'dlseller' ) . '</th><td>' . esc_html( $metadata ) . '</td></tr>';
	endif;

	if ( $metadata = usces_dlseller_filename( $post ) ) :
		$dlseller_table .= '<tr><th>' . __( 'File Name', 'dlseller' ) . '</th><td>' . esc_html( $metadata ) . '</td></tr>';
	endif;
	if ( $metadata = usces_get_itemMeta( '_dlseller_date', $post->ID, 'return' ) ) :
		$dlseller_table .= '<tr><th>' . __( 'Release Date', 'dlseller' ) . '</th><td>' . esc_html( $metadata ) . '</td></tr>';
	endif;
	if ( $metadata = usces_get_itemMeta( '_dlseller_version', $post->ID, 'return' ) ) :
		$dlseller_table .= '<tr><th>' . __( 'Version', 'dlseller' ) . '</th><td>' . esc_html( $metadata ) . '</td></tr>';
	endif;
	if ( $metadata = usces_get_itemMeta( '_dlseller_author', $post->ID, 'return' ) ) :
		$dlseller_table .= '<tr><th>' . __( 'Author', 'dlseller' ) . '</th><td>' . esc_html( $metadata ) . '</td></tr>';
	endif;
...
?>
							<p class="p-entry-item__cart-skuname"><?php usces_the_itemSkuDisp(); ?></p>
<?php
		endif;

		if ( usces_the_itemCprice( 'return' ) > 0 && usces_the_itemCprice( 'return' ) != usces_the_itemPrice( 'return' ) ) :
?>
							<p class="p-entry-item__cart-cprice"><?php _e( 'List price', 'tcd-w' ); ?>&nbsp;&nbsp;<?php if ( 'continue' == dlseller_get_charging_type( $post->ID ) ) : ?><span class="p-entry-item__cart-price-frequency"><?php dlseller_frequency_name( $post->ID, 'amount' ); ?></span>&nbsp;&nbsp;<?php endif; ?><span class="p-entry-item__cprice"><?php usces_the_itemCpriceCr(); ?></span><?php usces_guid_tax(); ?></p>
<?php
		endif;
?>
							<p class="p-entry-item__cart-price"><?php _e( 'Selling price', 'tcd-w' ); ?>&nbsp;&nbsp;<?php if ( 'continue' == dlseller_get_charging_type( $post->ID ) ) : ?><span class="p-entry-item__cart-price-frequency"><?php dlseller_frequency_name( $post->ID, 'amount' ); ?></span>&nbsp;&nbsp;<?php endif; ?><span class="p-entry-item__price p-price"><?php usces_the_itemPriceCr(); ?></span><?php usces_guid_tax(); ?></p>
<?php
		usces_the_itemGpExp();
...
<?php
		...
		if ( usces_have_zaiko() ) :
?>
							<p class="p-entry-item__cart-button"><?php usces_the_itemSkuButton( __( 'Add to Cart', 'tcd-w' ), 0 ); ?></p>
<?php
		else :
?>
							<p class="p-entry-item__cart-soldout"><?php _e( 'SOLD OUT', 'tcd-w' ); ?></p>
<?php
		endif;
?>
